...
function Record(props) {
    ...
    const startDate = hike.hike.start_point_datetime.format("DD/MM/YYYY")
    const startHour = hike.hike.start_point_datetime.format("HH:mm:ss")
    const endHour = hike.hike.end_point_datetime.format("HH:mm:ss")
    ...
    return (<>
      <Card style={{ width: '21rem' }} key={0} title={hike.hike.title}>
        <Card.Body>
          ...
          <h6>Starting Date: {startDate} </h6>
  
        </Card.Body>
        <ListGroup className="list-group-flush">
          <ListGroup.Item>Starting Time: {startHour} min</ListGroup.Item>
          <ListGroup.Item>Ending Time: {endHour} min</ListGroup.Item>
